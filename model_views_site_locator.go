/*
F5 Distributed Cloud Services API for ves.io.schema.views.origin_pool

Origin pool is a view to create cluster and endpoints that can be used in HTTP loadbalancer or TCP loadbalancer  It will create following child objects  * cluster * endpoints * healthcheck

API version: 
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// checks if the ViewsSiteLocator type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ViewsSiteLocator{}

// ViewsSiteLocator This message defines a reference to a site or virtual site object
type ViewsSiteLocator struct {
	Site *SchemaviewsObjectRefType `json:"site,omitempty"`
	VirtualSite *SchemaviewsObjectRefType `json:"virtual_site,omitempty"`
}

// NewViewsSiteLocator instantiates a new ViewsSiteLocator object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewViewsSiteLocator() *ViewsSiteLocator {
	this := ViewsSiteLocator{}
	return &this
}

// NewViewsSiteLocatorWithDefaults instantiates a new ViewsSiteLocator object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewViewsSiteLocatorWithDefaults() *ViewsSiteLocator {
	this := ViewsSiteLocator{}
	return &this
}

// GetSite returns the Site field value if set, zero value otherwise.
func (o *ViewsSiteLocator) GetSite() SchemaviewsObjectRefType {
	if o == nil || IsNil(o.Site) {
		var ret SchemaviewsObjectRefType
		return ret
	}
	return *o.Site
}

// GetSiteOk returns a tuple with the Site field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ViewsSiteLocator) GetSiteOk() (*SchemaviewsObjectRefType, bool) {
	if o == nil || IsNil(o.Site) {
		return nil, false
	}
	return o.Site, true
}

// HasSite returns a boolean if a field has been set.
func (o *ViewsSiteLocator) HasSite() bool {
	if o != nil && !IsNil(o.Site) {
		return true
	}

	return false
}

// SetSite gets a reference to the given SchemaviewsObjectRefType and assigns it to the Site field.
func (o *ViewsSiteLocator) SetSite(v SchemaviewsObjectRefType) {
	o.Site = &v
}

// GetVirtualSite returns the VirtualSite field value if set, zero value otherwise.
func (o *ViewsSiteLocator) GetVirtualSite() SchemaviewsObjectRefType {
	if o == nil || IsNil(o.VirtualSite) {
		var ret SchemaviewsObjectRefType
		return ret
	}
	return *o.VirtualSite
}

// GetVirtualSiteOk returns a tuple with the VirtualSite field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ViewsSiteLocator) GetVirtualSiteOk() (*SchemaviewsObjectRefType, bool) {
	if o == nil || IsNil(o.VirtualSite) {
		return nil, false
	}
	return o.VirtualSite, true
}

// HasVirtualSite returns a boolean if a field has been set.
func (o *ViewsSiteLocator) HasVirtualSite() bool {
	if o != nil && !IsNil(o.VirtualSite) {
		return true
	}

	return false
}

// SetVirtualSite gets a reference to the given SchemaviewsObjectRefType and assigns it to the VirtualSite field.
func (o *ViewsSiteLocator) SetVirtualSite(v SchemaviewsObjectRefType) {
	o.VirtualSite = &v
}

func (o ViewsSiteLocator) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ViewsSiteLocator) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Site) {
		toSerialize["site"] = o.Site
	}
	if !IsNil(o.VirtualSite) {
		toSerialize["virtual_site"] = o.VirtualSite
	}
	return toSerialize, nil
}

type NullableViewsSiteLocator struct {
	value *ViewsSiteLocator
	isSet bool
}

func (v NullableViewsSiteLocator) Get() *ViewsSiteLocator {
	return v.value
}

func (v *NullableViewsSiteLocator) Set(val *ViewsSiteLocator) {
	v.value = val
	v.isSet = true
}

func (v NullableViewsSiteLocator) IsSet() bool {
	return v.isSet
}

func (v *NullableViewsSiteLocator) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableViewsSiteLocator(val *ViewsSiteLocator) *NullableViewsSiteLocator {
	return &NullableViewsSiteLocator{value: val, isSet: true}
}

func (v NullableViewsSiteLocator) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableViewsSiteLocator) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


